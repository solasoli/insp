...
class SuratPerintahService
{
  ...
  private static function proccess_data(SuratPerintah $sp, $data)
  {
    ...
    DB::transaction(function () use ($sp, $data) {
      ...
      if ($type_pkpt->code == 'non_pkpt_tim' || $type_pkpt->code == 'non_pkpt_banyak_tim' || $type_pkpt->code == 'non_pkpt_non_tim')  { // Surat Perintah Non PKPT

        // prepare data kegiatan
        $data_kegiatan = [
          'kegiatan' => $input['kegiatan'],
          'wilayah' => isset($input['wilayah']) ? $input['wilayah'] : [],
          'opd' => isset($input['opd_proker']) ? $input['opd_proker'] : [],
          'dari' => $input['dari'],
          'sampai' => $input['sampai'],
          'sasaran' => !is_null($input['sasaran']) ? $input['sasaran'] : '',
          'jenis_pengawasan' => $input['jenis_pengawasan'],
          'all_opd' => isset($input['all_opd']) && $input['all_opd'] == 1 ? 1 : 0,
          'lintas_irban' => isset($input['lintas_irban']) && $input['lintas_irban'] == 1 ? 1 : 0,
        ...
        $input['wilayah'] = $program_kerja->wilayah->pluck('id')->toJson();
      ...
      $t->is_lampiran = isset($input['jadikan_lampiran']) && $input['jadikan_lampiran'] == 1 ? 1 : 0;
      $t->untuk = '';
      $t->dari = $data['dari'];
