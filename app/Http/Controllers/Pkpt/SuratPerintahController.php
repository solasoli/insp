...
class SuratPerintahController extends Controller
{
  ...
  {
    $data = SuratPerintah::with(['wilayah', 'program_kerja', 'sasaran', 'kegiatan', 'type_pkpt'])->where('is_approve', $approved)->select("pkpt_surat_perintah.*");
  ...
  private function get_file_sp($type) {
    ...
    switch($type_pkpt->code) {
      ...
      case 'non_pkpt_tim':
        ...
        break;
      case 'non_pkpt_banyak_tim':
        $surat_perintah_file = 'surat_perintah_non_pkpt_banyak_tim-form';
        $multiple_pkpt = true;
        break;
      case 'non_pkpt_non_tim':
        $surat_perintah_file = 'surat_perintah_non_pkpt_non_tim-form';
        break;
    } 
